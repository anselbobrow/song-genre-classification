...
def get_categorizer_option_parser():
    ...
    parser.add_option("-t",
                      "--train",
                      dest="train",
                      help="learn from training events in FILE",
                      metavar="FILE")
    parser.add_option("-p",
                      "--predict",
                      dest="predict",
                      help="predict labels for events in FILE",
                      metavar="FILE")
    parser.add_option("-o",
                      "--out",
                      dest="out",
    ...
    parser.add_option("-l",
                      ...
                      type="float",
                      default=0,
                      help="lambda value used in smoothing",
                      metavar="FLOAT")
    parser.add_option("-v",
                      "--verbose",
                      action="store_true",
...
def get_twitter_option_parser():
    ...
    parser.add_option(
        ...
        dest="auxtrain",
        help=
        "use auxiliary training events in FILE in addition to those specified with --train",
        metavar="FILE")
    parser.add_option("-e",
                      "--eval",
                      dest="eval",
    ...
    parser.add_option(
        ...
        default="mx",
        help=
        "type of model to use: maj (majority class baseline), lex (lexicon ratio baseline), nb (naive Bayes), pt (perceptron), mx (maximum entropy, default)",
        metavar="MODELTYPE")
    parser.add_option(
        "-r",
        "--sthreshold",
        dest="subjectivity_threshold",
        type="float",
        default=0.5,
        help=
        "during subjectivity classification, the confidence of the classifier must be greater than this value in order for the item to be labeled as subjective (default = 0.5)",
        metavar="FLOAT")
    parser.add_option(
        "-p",
        "--pthreshold",
        dest="polarity_threshold",
        type="float",
        default=0.3,
        help=
        "during polarity classification, the confidence of the classifier must be greater than this value in order for the item to be labeled at all (default = 0.3)",
        metavar="FLOAT")
    parser.add_option(
        "-s",
        "--smoothing",
        dest="smoothing_value",
        type="float",
        default=1.0,
        help=
        "value used in smoothing, either lambda for naive Bayes or sigma for maxent (default = 1.0)",
        metavar="FLOAT")
    parser.add_option(
        "-w",
        "--twostage",
    ...
    parser.add_option(
        ...
        default=False,
        help=
        "print detailed output regarding correct and incorrect classifications")
    return parser
...
def train_and_classify(training_events,
                       eval_events,
                       model_type,
                       smoothing_parameter,
                       verbose=False):
...
def run_opennlp(training_filename, eval_filename, model_type,
                smoothing_parameter, verbose):
    ...
    os.system('classify apply ' + model_file.name + ' ' + eval_filename + ' > '
              + predictions_file.name)

    # Pull predictions from outputfile and insert labels back into tweetset
    return [
        tuple(x.decode().strip().split(' ')[0:2])
        for x in predictions_file.readlines()
    ]


def run_naivebayes(training_filename, eval_filename, smoothing_parameter,
                   verbose):
